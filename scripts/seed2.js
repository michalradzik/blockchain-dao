...
async function main() {
  ...
  const daoAbi = require('../artifacts/contracts/MyDAO.sol/MyDAO.json').abi;
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await dao.connect(investor1).voteOnProposal(i + 1, true);
    await dao.connect(investor2).voteOnProposal(i + 1, true);
    await dao.connect(investor3).voteOnProposal(i + 1, true);
  ...
  await dao.connect(investor2).voteOnProposal(4, true);
  await dao.connect(investor3).voteOnProposal(4, true);
}
